...
class PortalModel(
    ...
    def _save_pretrained(self, save_directory: Path) -> None:
        ...
        tensors.update(
            {f"core.{name}": value.detach().cpu().contiguous() for name, value in self.decoder.core.state_dict().items()}
        )
        tensors.update(
            {
    ...
    def _from_pretrained(
        ...
        **hub_kwargs: Any,
        ...
        alignment_state = {
            name.removeprefix("alignment."): value
            for name, value in tensors.items()
            if name.startswith("alignment.")
        }
        if len(core_state) + len(alignment_state) != len(tensors):
            unexpected = sorted(set(tensors) - {f"core.{name}" for name in core_state} - {f"alignment.{name}" for name in alignment_state})
