...
class PortalConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.target_modules) != len(set(self.target_modules)) or set(self.target_modules) != expected_targets:
            raise ValueError(f"target_modules must correspond exactly to module_paths; expected {sorted(expected_targets)}")
    ...
    def shared_signature(self) -> tuple[Any, ...]:
        ...
        return (tuple(self.tasks), self.modules, self.rank, self.alpha, self.d_z, self.d_layer, self.hidden, self.d_core)
